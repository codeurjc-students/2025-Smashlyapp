...
describe('RacketService - Unit Tests', () => {
  ...
  describe('Funcionalidad de Filtrado', () => {
    ...
    test('should filter rackets by price range correctly', () => {
      ...
      const filterByPriceRange = (rackets: typeof mockRacketData, minPrice: number, maxPrice: number) => {
    ...
    test('should filter rackets by playing level correctly', () => {
      ...
      const filterByLevel = (rackets: typeof mockRacketData, level: string) => {
        return rackets.filter(racket => racket.caracteristicas_nivel_de_juego === level);
      };
  ...
  describe('Funcionalidad de Búsqueda', () => {
    ...
    test('should handle case insensitive search', () => {
      ...
      const caseInsensitiveSearch = (rackets: typeof mockRacketData, query: string) => {
        return rackets.filter(racket => 
          racket.nombre.toLowerCase().includes(query.toLowerCase()) ||
          racket.marca.toLowerCase().includes(query.toLowerCase())
        );
  ...
  describe('Cálculos de Precios y Descuentos', () => {
    test('should calculate discounts correctly', () => {
      ...
      const bullpadelRacket = mockRacketData.find(r => r.marca === 'BULLPADEL');
      const adidasRacket = mockRacketData.find(r => r.marca === 'ADIDAS');

      if (noxRacket) {
        const discount = calculateDiscount(noxRacket.precio_original, noxRacket.precio_actual);
        expect(discount).toBe(48); // 48% descuento
      }

      if (bullpadelRacket) {
        const discount = calculateDiscount(bullpadelRacket.precio_original, bullpadelRacket.precio_actual);
        expect(discount).toBe(0); // Sin descuento
      }

      if (adidasRacket) {
        const discount = calculateDiscount(adidasRacket.precio_original, adidasRacket.precio_actual);
    ...
    test('should identify best deals correctly', () => {
      ...
      expect(bestDeals[0].descuento_porcentaje).toBeGreaterThan(bestDeals[1].descuento_porcentaje);
    });
  });
  ...
  describe('Estadísticas y Agregaciones', () => {
    test('should calculate basic statistics correctly', () => {
      ...
      const getStats = (rackets: typeof mockRacketData) => {
        ...
        const avgPrice = rackets.reduce((sum, r) => sum + r.precio_actual, 0) / total;
  ...
  describe('Validación de Datos', () => {
    test('should validate required fields', () => {
      ...
      const validateRacket = (racket: any) => {
        ...
        return requiredFields.every(field => racket[field] !== undefined && racket[field] !== null);
      };
  ...
  describe('Paginación y Ordenamiento', () => {
    test('should paginate results correctly', () => {
      ...
      const paginate = (rackets: typeof mockRacketData, page: number, limit: number) => {
    ...
    test('should sort rackets correctly', () => {
      ...
      const sortRackets = (rackets: typeof mockRacketData, field: keyof typeof mockRacketData[0], order: 'asc' | 'desc') => {
      ...
      const sortedByPriceAsc = sortRackets(mockRacketData, 'precio_actual', 'asc');
      const sortedByPriceDesc = sortRackets(mockRacketData, 'precio_actual', 'desc');
