...
### 🧭 Diagrama detallado (similar al de la imagen)

```mermaid
flowchart LR
  %% Columnas por capas
  subgraph V[Views (Frontend)]
    VLogin[LoginPage.tsx]
    VRegister[RegisterPage.tsx]
    VAdmin[Admin pages]
    VProfile[Profile pages]
  end

  subgraph R[Routes (Express)]
    RAuth[auth.ts\nPOST /api/auth/register\nPOST /api/auth/login]
    RUsers[users.ts\nGET /api/v1/users/me]
    RAdmin[admin.ts\nPATCH /api/v1/admin/users/:id/role]
    RStores[stores.ts\nPOST /api/v1/stores/requests]
  end

  subgraph M[Middleware]
    MValidation[validation.ts\nJoi schemas]
    MAuth[auth.ts\nJWT + perfil]
    MRequireAdmin[requireAdmin.ts]
  end

  subgraph C[Controllers]
    CAuth[AuthController]
    CUsers[UserController]
    CAdmin[AdminController]
    CStores[StoreController]
  end

  subgraph S[Services]
    SUsers[userService]
    SStores[storeService]
    SAdmin[adminService]
  end

  subgraph D[Data Access]
    DSupabase[supabase.ts\nclient]
  end

  subgraph DB[Database / Tables]
    TProfiles[(user_profiles)]
    TStores[(stores)]
    TStoreReq[(store_requests)]
    TRackets[(rackets)]
  end
  ...
  %% Estilos por capa (colores aproximados)
  classDef views fill:#eef7ff,stroke:#6aa9ff,stroke-width:1px;
  classDef routes fill:#e8f5e9,stroke:#43a047,stroke-width:1px;
  classDef middleware fill:#fff8e1,stroke:#fbc02d,stroke-width:1px;
  classDef controllers fill:#fde7e9,stroke:#e57373,stroke-width:1px;
  classDef services fill:#ede7f6,stroke:#7e57c2,stroke-width:1px;
  classDef dataaccess fill:#e0f2f1,stroke:#26a69a,stroke-width:1px;
  classDef tables fill:#f3e5f5,stroke:#ab47bc,stroke-width:1px;

  class VLogin,VRegister,VAdmin,VProfile views;
  class RAuth,RUsers,RAdmin,RStores routes;
  class MValidation,MAuth,MRequireAdmin middleware;
  class CAuth,CUsers,CAdmin,CStores controllers;
  class SUsers,SStores,SAdmin services;
  class DSupabase dataaccess;
  class TProfiles,TStores,TStoreReq,TRackets tables;
```

> Este diagrama refleja capas y dependencias: vistas del frontend → rutas → middleware → controladores → servicios → acceso a datos → tablas de Supabase.
